...
export default function GearClient({ initialGears, categories }: { initialGears: any[], categories: any[] }) {
  ...
  const filteredGears = useMemo(() => {
    return initialGears.filter((gear) => {
      ...
      const matchesCategory = selectedCategory === "all" || gear.categoryId === selectedCategory;
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8 bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
       ...
       <div className="flex items-center gap-2 overflow-x-auto md:flex-wrap pb-2 md:pb-0 hide-scrollbar scroll-smooth">
          <Button
            variant={selectedCategory === "all" ? "default" : "outline"}
            onClick={() => handleFilterChange("all")}
            className={`rounded-xl whitespace-nowrap h-10 md:h-12 text-sm ${selectedCategory === "all" ? "bg-emerald-600 hover:bg-emerald-700 text-white" : "dark:text-slate-300 dark:border-slate-700"}`}
          >
            All Gear
          </Button>
          {categories.map(cat => (
            <Button
              ...
              className={`rounded-xl whitespace-nowrap h-10 md:h-12 text-sm ${selectedCategory === cat.id ? "bg-emerald-600 hover:bg-emerald-700 text-white" : "dark:text-slate-300 dark:border-slate-700"}`}
            >
              {cat.name}
            </Button>
          ))}
      ...
      {paginatedGears.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {paginatedGears.map((gear) => (
            <Link href={`/gear/${gear.id}`} key={gear.id} className="group flex flex-col h-full bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-md transition-all">
              {/* Image Placeholder */}
              ...
              <div className="p-5 flex flex-col flex-1">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800 mt-auto">
                  ...
                  <Button variant="ghost" className="text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 rounded-xl">
                    View Details
                  </Button>
                </div>
              </div>
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">No gear found</h3>
          <p className="text-slate-500 dark:text-slate-400 max-w-sm">
            We couldn't find anything matching your search or filter criteria. Try adjusting them.
          </p>
          <Button onClick={() => {setSearchTerm(""); setSelectedCategory("all")}} className="mt-6 bg-emerald-600 text-white hover:bg-emerald-700">
            Clear Filters
          </Button>
        </div>
      )}
